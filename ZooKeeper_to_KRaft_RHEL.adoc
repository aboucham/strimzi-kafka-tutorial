...
[source, bash]
----
export CLUSTER_ID=SY9T31EKSS6XeNO_Jio6yQ
./bin/kafka-storage.sh format --cluster-id $CLUSTER_ID --standalone --config ./config/kraft/controller-100.properties
./bin/kafka-storage.sh format --cluster-id $CLUSTER_ID --standalone --config ./config/kraft/controller-101.properties
./bin/kafka-storage.sh format --cluster-id $CLUSTER_ID --standalone --config ./config/kraft/controller-102.properties
----
...
----
    ...
    **Verification (logs):** Confirm the state has been changed from NON to PRE_MIGRATION on all three controllers.

[source, bash]
----
grep "Replayed a ZkMigrationStateRecord changing the migration state from NONE to PRE_MIGRATION" ./logs/controller.log
[2025-11-19 12:54:57,847] INFO [QuorumController id=100] Replayed a ZkMigrationStateRecord changing the migration state from NONE to PRE_MIGRATION. (org.apache.kafka.controller.FeatureControlManager)
[2025-11-19 12:54:59,058] INFO [QuorumController id=101] Replayed a ZkMigrationStateRecord changing the migration state from NONE to PRE_MIGRATION. (org.apache.kafka.controller.FeatureControlManager)
[2025-11-19 12:55:00,231] INFO [QuorumController id=102] Replayed a ZkMigrationStateRecord changing the migration state from NONE to PRE_MIGRATION. (org.apache.kafka.controller.FeatureControlManager)
----

*NOTE*: *PRE_MIGRATION* (the state where KRaft is preparing to ingest metadata from ZooKeeper).
...
[source, bash]
----
grep "Replayed a ZkMigrationStateRecord changing the migration state from PRE_MIGRATION to MIGRATION" ./logs/controller.log
[2025-11-19 13:02:10,383] INFO [QuorumController id=100] Replayed a ZkMigrationStateRecord changing the migration state from PRE_MIGRATION to MIGRATION. (org.apache.kafka.controller.FeatureControlManager)
----
...
Once those logs entries appears, the cluster is operating stably in the **Dual-Write** mode.
...
[source, bash]
----
grep "Replayed a ZkMigrationStateRecord changing the migration state from MIGRATION to POST_MIGRATION." ./logs/controller.log
[2025-11-19 13:21:33,606] INFO [QuorumController id=100] Replayed a ZkMigrationStateRecord changing the migration state from MIGRATION to POST_MIGRATION. (org.apache.kafka.controller.FeatureControlManager)
----

That log entry confirms the successful transition of your KRaft controller quorum to the final phase of the migration process! 🎉

*POST_MIGRATION:* This is the terminal state of the migration process. It signals that the KRaft controllers are now considered the *exclusive source of truth* for cluster metadata, and the cluster is logically operating in KRaft mode.
Once completed, your Kafka cluster is fully operating in KRaft mode. *You can now decommission the ZooKeeper nodes*.

**Warning:** Once the migration is finalized, **rollback to ZooKeeper is no longer possible**.
